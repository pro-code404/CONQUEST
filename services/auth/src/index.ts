...
/** Authentication & Authorization — least privilege, audit-ready */
export class AuthService {
  ...
  authenticate(headers: Record<string, string | undefined>): AuthContext {
    ...
    if (apiKey && this.config.apiKey && apiKey.length === this.config.apiKey.length
    ...
    if (authHeader?.startsWith("Bearer ")) {
      ...
      try {
        ...
        throw new ConquestError(ConquestErrorCode.AUTHENTICATION_FAILED, "Invalid token", false, FailureClass.Internal);
      }
    }
  ...
  authorize(ctx: AuthContext, requiredScope: string): void {
    if (!ctx.scopes.includes(requiredScope) && !ctx.scopes.includes("admin")) {
      throw new ConquestError(ConquestErrorCode.AUTHORIZATION_FAILED, `Missing scope: ${requiredScope}`, false, FailureClass.Internal);
    }
  }
